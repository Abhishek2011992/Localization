...
Eigen::Matrix4d NormalDistTransform(pcl::NormalDistributionsTransform<pcl::PointXYZ, pcl::PointXYZ> ndt, PointCloudT::Ptr source, Pose startingPose, int iterations)
{
...
int main(){
	...
	renderPointCloud(viewer, mapCloud, "map", Color(0,0,1)); 
  	...
  	/*initialize NDT only once with the mapCloud*/
	...
	while (!viewer->wasStopped())
  	{
		...
		if(!new_scan){
            ...
            pose = Pose(Point(vehicle->GetTransform().location.x, vehicle->GetTransform().location.y, vehicle->GetTransform().location.z), Rotate(vehicle->GetTransform().rotation.yaw * pi/180, vehicle->GetTransform().rotation.pitch * pi/180, vehicle->GetTransform().rotation.roll * pi/180)) - poseRef;
       		...
       		Eigen::Matrix4d transform = NormalDistTransform(ndt, cloudFiltered, pose, 5);
			// TODO: Transform scan so it aligns with ego's actual pose and render that scan
			typename pcl::PointCloud<PointT>::Ptr transformedscan (new pcl::PointCloud<PointT>);
          	pcl::transformPointCloud (*scanCloud, *transformedscan, transform);
